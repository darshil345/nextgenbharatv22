...
import { BookOpen, FileText, Clock, ChevronRight, ChevronDown, ArrowLeft, Sparkles, GraduationCap, Zap } from "lucide-react";
        ...
        <div className="prose prose-sm max-w-none">
          {note.content.split("\n").map((line, i) => {
            if (line.startsWith("**") && line.endsWith("**")) {
              return <h3 key={i} className="font-heading font-bold text-foreground text-base mt-4 mb-2">{line.replace(/\*\*/g, "")}</h3>;
            }
            if (line.startsWith("**")) {
            ...
            if (line.startsWith("•")) {
              return <p key={i} className="text-sm text-muted-foreground ml-3 mb-0.5">{line}</p>;
            }
            if (line.match(/^\d+\./)) {
              return <p key={i} className="text-sm text-foreground ml-3 mb-0.5">{line}</p>;
            }
            if (line.startsWith("→")) {
              return <p key={i} className="text-sm text-primary font-semibold ml-3 mb-1">{line}</p>;
            }
...
const Practice = () => {
  ...
  if (selectedNote) {
    return (
      <div className="max-w-2xl mx-auto">
        <NoteViewer note={selectedNote} onBack={() => setSelectedNote(null)} />
      </div>
    );
  }
  ...
  if (selectedCategory) {
    return (
      <div className="max-w-2xl mx-auto">
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
          <Button variant="ghost" size="sm" onClick={() => setSelectedCategory(null)} className="mb-3 gap-1 text-muted-foreground">
            <ArrowLeft size={14} /> All Topics
          </Button>

          <div className="flex items-center gap-3 mb-5">
            <div className="w-12 h-12 rounded-2xl gradient-hero flex items-center justify-center text-2xl">
          ...
          <Tabs defaultValue="notes" className="w-full">
            <TabsList className="w-full mb-4">
              ...
              <TabsTrigger value="worksheets" className="flex-1 gap-1">
                <FileText size={14} /> Worksheets
              </TabsTrigger>
            </TabsList>
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3 text-xs text-muted-foreground">
                          <span className="flex items-center gap-1">
                            <FileText size={12} /> {ws.questionCount} questions
                          </span>
                          <span className="flex items-center gap-1">
                            <Clock size={12} /> {ws.estimatedTime}
                          </span>
                        </div>
          ...
          >
            ...
            >
              <CardContent className="p-4 flex items-center gap-4">
                <div className="w-12 h-12 rounded-2xl gradient-hero flex items-center justify-center text-2xl shrink-0">
                  {cat.icon}
                </div>
                <div className="flex-1">
                  ...
                  <div className="flex gap-2 mt-1.5">
                    <Badge variant="outline" className="text-[10px] gap-1">
                      <BookOpen size={10} /> {cat.notes.length} notes
                    </Badge>
                    <Badge variant="outline" className="text-[10px] gap-1">
                      <FileText size={10} /> {cat.worksheets.length} worksheets
                    </Badge>
                  </div>
                </div>
