...
import { INAIO_SYLLABUS, type Stage } from "@/data/inaiOSyllabus";
...
const STAGE_COLORS = ["gradient-blue", "gradient-orange", "gradient-hero", "from-purple-500 to-pink-500"];
const SECTION_COLORS = ["text-primary", "text-secondary", "text-accent"];

const SyllabusContent = ({ syllabus, label }: { syllabus: Stage[]; label: string }) => (
  <div className="space-y-6">
    {syllabus.map((stage, stageIdx) => (
      <motion.div
        key={stage.title}
        ...
        <Card className="overflow-hidden">
          <CardHeader className="pb-3">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl ${STAGE_COLORS[stageIdx % STAGE_COLORS.length]} flex items-center justify-center shrink-0`}>
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-muted-foreground text-sm mb-4">
          Choose your track and master every topic.
        </p>
      </motion.div>
      ...
      <Tabs defaultValue="inaio" className="w-full">
        ...
        <TabsContent value="inaio">
          <SyllabusContent syllabus={INAIO_SYLLABUS} label="INAIO" />
        </TabsContent>
        <TabsContent value="ioai">
          <SyllabusContent syllabus={IOAI_SYLLABUS} label="IOAI" />
        </TabsContent>
      </Tabs>
    </div>
  );
