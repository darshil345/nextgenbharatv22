...
import { ROADMAP_STAGES, type RoadmapStage } from "@/data/roadmapData";
import { IOAI_ROADMAP_STAGES } from "@/data/ioaiRoadmapData";
import { supabase } from "@/integrations/supabase/client";
import { useAuth } from "@/contexts/AuthContext";

const RoadmapPath = ({ stages, completedExercises, finishLabel }: { stages: RoadmapStage[]; completedExercises: Set<string>; finishLabel: string }) => {
  const allExerciseIds: string[] = [];
  stages.forEach(stage => stage.nodes.forEach(node => node.exercises.forEach(ex => allExerciseIds.push(ex.id))));
  ...
  const allNodes = stages.flatMap((stage, si) =>
  ...
  return (
    <div className="max-w-lg mx-auto pb-12">
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="text-center mb-8">
        ...
        <h1 className="font-heading text-2xl md:text-3xl font-bold text-foreground">Your Learning Path</h1>
      ...
      <div className="relative">
        ...
        {allNodes.map((node, idx) => {
          ...
          return (
            <div key={node.id}>
              {showStageTitle && (
                <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: idx * 0.05 }} className="relative z-10 flex justify-center my-6">
              ...
              >
                <div className="relative z-10 shrink-0">
                  <div className={`w-16 h-16 rounded-full flex items-center justify-center text-2xl border-4 shadow-lg transition-all duration-300 ${
                    allDone ? "border-accent bg-accent/10 shadow-accent/20"
                      : isActive ? "border-primary bg-primary/10 shadow-primary/30 animate-pulse"
                  ...
                  <div className="flex gap-1 justify-center mt-1.5">
                    {nodeExercises.map((ex) => (
                      <div key={ex.id} className={`w-2 h-2 rounded-full transition-colors ${
                        completedExercises.has(ex.id) ? "bg-accent" : isUnlocked(ex.id) ? "bg-primary/50" : "bg-muted-foreground/20"
                      }`} />
                    ))}
                  </div>
                </div>
                ...
                <div className={`flex-1 rounded-2xl border-2 p-3 transition-all ${
                  allDone ? "border-accent/30 bg-accent/5"
                    : isActive ? "border-primary/30 bg-card shadow-md hover:shadow-lg"
                  ...
                  <p className="text-xs text-muted-foreground mb-2">{completedCount}/{nodeExercises.length} exercises</p>
                  <div className="flex flex-wrap gap-1.5">
                    {nodeExercises.map((exercise) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-xs font-semibold transition-all ${
                            completed ? "bg-accent/15 text-accent border border-accent/30"
                              : unlocked ? "bg-primary/10 text-primary border border-primary/30 hover:bg-primary/20 hover:scale-105 cursor-pointer"
                              : "bg-muted text-muted-foreground border border-border"
                          }`}>
                            {completed ? <CheckCircle2 size={12} /> : unlocked ? <Star size={12} /> : <Lock size={10} />}
                            {exercise.title}
                            <Badge className={`ml-1 text-[9px] px-1 py-0 h-4 ${
                              exercise.difficulty === "Hard" ? "bg-destructive/10 text-destructive border-destructive/20"
                                : exercise.difficulty === "Medium" ? "bg-secondary/10 text-secondary border-secondary/20"
                                : "bg-accent/10 text-accent border-accent/20"
                            }`}>+{exercise.xpReward}</Badge>
                          </div>
                        </Link>
                      );
                    })}
        ...
        <motion.div initial={{ opacity: 0, scale: 0 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 1.5, type: "spring" }} className="relative flex justify-center mt-4">
        ...
        <p className="text-center text-sm font-heading font-bold text-muted-foreground mt-2">{finishLabel}</p>
      </div>
    </div>
  );
};
...
const Roadmap = () => {
  ...
  return (
    <Tabs defaultValue="inaio" className="w-full">
      ...
      <TabsContent value="inaio">
        <RoadmapPath stages={ROADMAP_STAGES} completedExercises={completedExercises} finishLabel="INAIO Ready!" />
      </TabsContent>
      <TabsContent value="ioai">
        <RoadmapPath stages={IOAI_ROADMAP_STAGES} completedExercises={completedExercises} finishLabel="IOAI Ready!" />
      </TabsContent>
    </Tabs>
  );
};
